...
namespace UnityEngine.U2D.Common
{
    ...
    internal class TriangulationUtility
    {
        ...
        [BurstCompile]
        public static bool TessellateMainThreadBurst(Allocator allocator, ref NativeArray<float2> points, ref NativeArray<int2> edges, out NativeArray<float2> outVertices, out NativeArray<int> outIndices)
        {
            // For Main-Thread only and also this only allocates minimal memory.
            return ModuleHandle.TessellateMainThread(allocator, ref points, ref edges, out outVertices, out outIndices);
        }
        ...
        internal static bool TriangulateSafe(ref float2[] vertices, ref int2[] edges, out int[] indices)
        {
            ...
            NativeArray<float2> outputVertices;
            int arrayCount = vertices.Length * vertices.Length * 4;

            bool ok = TessellateMainThreadBurst(Allocator.Temp, ref points, ref inputEdges, out outputVertices, out outputIndices);
